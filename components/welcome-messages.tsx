...
function MessageCard({ message, isFr }: { message: WelcomeMessage; isFr: boolean }) {
  ...
  return (
    <article ref={cardRef} aria-labelledby={`${id}-title`} className="scroll-mt-36 overflow-hidden rounded-3xl border border-[#E8D4DB] bg-white p-6 shadow-[0_12px_40px_rgba(142,12,58,0.06)] sm:p-8">
      <header className="mb-5 flex flex-col gap-5 sm:flex-row sm:items-center sm:gap-7">
        ...
        <div className="min-w-0">
          {message.id === "cofounders" && <p className="text-xs font-bold uppercase tracking-[0.13em] text-[#8E0C3A]">{isFr ? message.labelFr : message.labelEn}</p>}
          <h3 id={`${id}-title`} className="mt-2 font-heading text-xl font-black leading-tight text-[#8E0C3A] sm:text-2xl">{title}</h3>
          {message.id !== "mayor" && message.id !== "province" && (
          <div className="mt-3 space-y-1 text-sm leading-relaxed text-[#8E0C3A]">
      ...
      <div className="relative overflow-hidden transition-[height] duration-500 ease-in-out motion-reduce:transition-none" style={{ height: expanded ? contentHeight : 112 }}>
        ...
        <div id={`${id}-content`} ref={contentRef} role="region" aria-labelledby={`${id}-title`} aria-hidden={!expanded} className={expanded ? "visible" : "invisible"}>
          ...
          <div className="mt-8 border-l-4 border-[#8E0C3A] pl-4 text-sm leading-relaxed text-[#8E0C3A] sm:text-base">
            {signature.map((line, index) => <p key={line} className={index === 0 ? "font-black" : ""}>{line}</p>)}
          </div>
          {message.logo && (
            <div className="mt-6">
              {message.logo === "foundation" ? (
                <Foundation role="img" aria-label={isFr ? "Fondation Michaëlle Jean" : "Michaëlle Jean Foundation"} className="h-auto w-[270px] max-w-full [&_path]:fill-[#8E0C3A]" />
              ) : (
    ...
    <section id="welcome-messages" className="scroll-mt-36 bg-[#FAF6F7] px-5 py-14 sm:py-20">
      <div className="mx-auto max-w-[1180px]">
        <div className="mb-9 max-w-3xl">
          <p className="font-heading text-sm font-bold uppercase tracking-[0.16em] text-[#8E0C3A]">{isFr ? "Bienvenue à Winnipeg" : "Welcome to Winnipeg"}</p>
