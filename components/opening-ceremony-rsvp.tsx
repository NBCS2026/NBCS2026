...
export function OpeningCeremonyRsvp({ locale }: { locale: string }) {
  ...
  return <section id="opening-ceremony-rsvp" className="rounded-2xl border-2 border-[#8E0C3A] bg-white p-5 sm:p-8">
    <h2 className="font-heading text-2xl font-bold text-[#8E0C3A]">{isFr ? "RSVP — Cérémonie d’ouverture" : "Opening Ceremony RSVP"}</h2>
    ...
    {ceremony.action && <a href={ceremony.action.url} target="_blank" rel="noopener noreferrer" className="mt-5 inline-flex min-h-11 items-center justify-center rounded-full bg-[#8E0C3A] px-6 py-3 text-center font-bold text-white hover:bg-[#8E0C3A]">{ceremony.action.label}</a>}
  </section>;
}
