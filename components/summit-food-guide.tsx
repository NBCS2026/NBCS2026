...
export function SummitFoodGuide({ locale, open }: { locale: string; open: boolean }) {
  ...
  const actionClass = "inline-flex min-h-11 items-center justify-center gap-2 rounded-full px-4 py-2.5 text-sm font-bold focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-[#8E0C3A]";
  ...
  return (
    <div className="min-w-0 space-y-6">
      ...
      <div className="overflow-hidden rounded-2xl border border-[#E8D4DB] bg-[#FAF6F7] sm:rounded-3xl">
        ...
        />}
        <a href={`https://www.google.com/maps/d/viewer?mid=${FOOD_MAP_ID}&hl=${isFr ? "fr" : "en"}`} target="_blank" rel="noopener noreferrer" className="flex min-h-11 items-center justify-center gap-2 px-4 py-3 text-center text-sm font-semibold text-[#8E0C3A] underline underline-offset-4">
        ...
        {SUMMIT_FOOD_BUSINESSES.map(business => (
          <li key={business.id} className="min-w-0">
            <article className="flex h-full min-w-0 flex-col rounded-2xl border border-[#E8D4DB] bg-white p-5 text-[#1E1E1E] sm:p-6" aria-labelledby={`food-${business.id}`}>
              <h3 id={`food-${business.id}`} className="font-heading text-lg font-bold leading-snug text-[#8E0C3A] sm:text-xl">{business.name}</h3>
              <p className="mt-2 text-sm leading-relaxed text-[#1E1E1E]/75">{isFr ? business.typeFr : business.typeEn}</p>
              <p className="mt-3 flex items-start gap-2 text-sm leading-relaxed"><MapPin className="mt-0.5 size-4 shrink-0 text-[#8E0C3A]" aria-hidden /><span>{business.address}</span></p>
              <p className="mb-4 mt-2 flex items-start gap-2 text-sm font-semibold leading-relaxed text-[#8E0C3A]"><Footprints className="mt-0.5 size-4 shrink-0" aria-hidden /><span>{business.walkMinutes
              ...
              <div className="mt-auto grid grid-cols-1 gap-2 sm:grid-cols-2">
                {business.website && <a href={business.website} target="_blank" rel="noopener noreferrer" aria-label={`${isFr ? "Site Web / Menu" : "Website / Menu"} — ${business.name}`} className={`${actionClass} border border-[#E8D4DB] bg-[#FAF6F7] text-[#8E0C3A] hover:bg-[#E8D4DB]`}><ExternalLink className="size-4 shrink-0" aria-hidden />{isFr ? "Site Web / Menu" : "Website / Menu"}</a>}
                <a href={foodDirectionsUrl(business)} target="_blank" rel="noopener noreferrer" aria-label={isFr ? `Itinéraire vers ${business.name} depuis le Centre des congrès RBC` : `Directions to ${business.name} from RBC Convention Centre`} className={`${actionClass} bg-[#8E0C3A] text-white transition-[filter] hover:brightness-110`}><Navigation className="size-4 shrink-0" aria-hidden />{isFr ? "Itinéraire" : "Directions"}</a>
              </div>
            </article>
          </li>
        ))}
