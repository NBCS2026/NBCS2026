          ...
          <li key={business.id} className="min-w-0">
            <article className="flex h-full min-w-0 flex-col rounded-2xl border border-[#E8D4DB] bg-white p-5 text-[#1E1E1E] sm:p-6" aria-labelledby={`food-${business.id}`}>
              ...
              <div className="mt-auto grid grid-cols-1 gap-2 sm:grid-cols-2">
                <a href={business.website} target="_blank" rel="noopener noreferrer" aria-label={`${isFr ? "Site Web / Menu" : "Website / Menu"} — ${business.name}`} className={`${actionClass} border border-[#E8D4DB] bg-[#FAF6F7] text-[#5D1831] hover:bg-[#E8D4DB]`}><ExternalLink className="size-4 shrink-0" aria-hidden />{isFr ? "Site Web / Menu" : "Website / Menu"}</a>
