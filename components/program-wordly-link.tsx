...
export function ProgramWordlyLink({ day, sessionId, title, isFr }: {
  ...
  if (!url) return null;
  const label = isFr ? "Interprétation simultanée et sous-titrage en direct" : "Simultaneous interpretation and live captioning";
  return <div className="program-wordly-wrap">
    <a className="program-wordly-link" href={url} target="_blank" rel="noopener noreferrer"
      aria-label={`${label} — ${title} (${isFr ? "nouvel onglet" : "new tab"})`}>
